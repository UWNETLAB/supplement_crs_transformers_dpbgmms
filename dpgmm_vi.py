...
"""
Note: very slightly modified version of the script available here: https://github.com/mcusi/tf_dpgmm
...
class dpgmm():
    ...
    def initialize_latents(self, X, batch_size, shared=True, use_mask=True):
        ...
        zeta_dist = tfp.distributions.Dirichlet(alpha_vec)
        #zeta: batch_size N T
...
def variational_inference(data, alpha=1.0, T=10, n_iter=10, tf_seed=None, get_elbo=False, tf_device='/cpu:0'):
    ...
    with tf.Graph().as_default():
        with tf.device(tf_device):
            ...
            init_a, init_b, init_lambda_1, init_lambda_2, init_nu, init_omega, init_zeta, mask = mixture_model.initialize_latents(X, batch_size, shared=True)
